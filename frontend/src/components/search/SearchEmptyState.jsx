...
const SearchEmptyState = ({ searchedService, searchedServiceIsListed, serviceUnavailable, suggestedServices, onSearchService, t }) => {
  // Case A: User searched for a known but currently inactive/unavailable service
  if (serviceUnavailable) {
    return (
      <div className="md:col-span-2 xl:col-span-3 bg-white rounded-3xl p-6 sm:p-10 text-center border border-slate-100 premium-shadow">
        <div className="w-14 h-14 bg-orange-50 text-orange-500 rounded-2xl mx-auto flex items-center justify-center mb-5">
          <AlertTriangle size={26} />
        </div>
        <p className="font-bold text-slate-800 text-xl">
          {t('search.serviceUnavailableTitle')}
        </p>
        <p className="text-slate-500 mt-2 max-w-2xl mx-auto">
          {t('search.serviceUnavailableMessage')}
        </p>
  ...
  // Case B: No workers found or completely unknown/unrelated search
  return (
    <div className="md:col-span-2 xl:col-span-3 bg-white rounded-3xl p-6 sm:p-10 text-center border border-slate-100 premium-shadow">
