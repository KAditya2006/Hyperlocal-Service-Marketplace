...
const WorkerProfile = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <Navbar />
      <main className="max-w-6xl mx-auto px-6 py-10 grid lg:grid-cols-[1fr_380px] gap-8">
        <section className="space-y-6">
          <div className="bg-white border border-slate-100 premium-shadow rounded-3xl p-8">
            <div className="flex flex-col md:flex-row gap-6">
              <img src={worker.user?.avatar} alt={worker.user?.name} className="w-28 h-28 rounded-3xl object-cover" />
              <div className="space-y-3">
                <h1 className="text-4xl font-bold font-heading text-slate-900">{worker.user?.name}</h1>
                ...
                <p className="flex items-center gap-2 text-slate-500"><MapPin size={18} /> {worker.user?.location?.address || 'Nearby'}</p>
        ...
        <aside className="bg-white border border-slate-100 premium-shadow rounded-3xl p-6 h-fit space-y-5">
